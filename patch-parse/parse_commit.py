...
def if_msg_security(msg):
    key_msgs = ['leak', 'security', 'vuln', 'fix', 'null deref', 'overflow', 'by zero', 'issue', 'crash', 'fault', 'abort', 'memory', 'code exec', 'bypass', 'cve ', 'cwe ']
    msg = msg.replace('-', ' ')
    msg = msg.replace('_', ' ')
    msg = msg.replace('.', ' ')
    msg = msg.replace(',', ' ')
...
# DEBUG
def if_commit_API1(m: ModifiedFile, fc_list, api_list):
    match_list = list()
    before_code = m.source_code_before
    after_code = m.source_code
    diff_info = m.diff_parsed
...
# DEBUG
def if_commit_API2(m: ModifiedFile, fc_list, api_list):
    window_size = 10
    before_code = m.source_code_before
    after_code = m.source_code
    diff_info = m.diff_parsed
...
# 最少的commit：只有修改行有API
# DEBUG
def if_commit_API3(m: ModifiedFile, fc_list, api_list):
    before_code = m.source_code_before
    after_code = m.source_code
    diff_info = m.diff_parsed
    match_api_list = list()
    ...
    for repo_dict in repo_list:
        ...
        if len(parse_repo) != 0:
            if repo_name not in parse_repo:
                continue
        api_list = list()
        api_path = api_dir + '/' + repo_name + '-apilist'
        ...
        for commit in Repository(repo).traverse_commits():
            ...
            if len(hit_list) != 0:
                ...
                for m in commit.modified_files:
                    ...
                    # print(tmp_list)
                    # exit(1)
                    match_list = if_commit_API1(m, fc_list, api_list)
                    if len(match_list) == 0:
                    ...
                    api_level = 1
                    tmp_list = match_list
                    match_list = if_commit_API2(m, fc_list, api_list)
                    if len(match_list) == 0:
                    ...
                    api_level = 2
                    tmp_list = match_list
                    match_list = if_commit_API3(m, fc_list, api_list)
                    if len(match_list) == 0:
